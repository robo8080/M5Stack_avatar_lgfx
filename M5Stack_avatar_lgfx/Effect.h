...
class Effect final : public Drawable {
 private:
  void drawBubbleMark(LGFX_Sprite *spi, uint32_t x, uint32_t y, uint32_t r,
                      uint16_t color) {
  ...
  void drawSweatMark(LGFX_Sprite *spi, uint32_t x, uint32_t y, uint32_t r,
                 uint16_t color) {
  ...
  void drawChillMark(LGFX_Sprite *spi, uint32_t x, uint32_t y, uint32_t r,
                     uint16_t color) {
  ...
  void drawAngerMark(LGFX_Sprite *spi, uint32_t x, uint32_t y, uint32_t r,
  ...
  void drawHeartMark(LGFX_Sprite *spi, uint32_t x, uint32_t y, uint32_t r,
                 uint16_t color) {
 ...
 public:
  ...
  void draw(LGFX_Sprite *spi, BoundingRect rect, DrawContext *ctx) override {
    uint16_t primaryColor = ctx->getColorPalette()->get(COLOR_PRIMARY);
    uint16_t bgColor = COLOR_DEPTH == 1 ? ERACER_COLOR : ctx->getColorPalette()->get(COLOR_BACKGROUND);
